...
def test_una_corrida_deshecha_lo_dice(monkeypatch):
    ...
    assert "Este totalizar después se deshizo" in PREVIEW
...
def test_la_ruta_existe_y_la_ve_quien_ve_la_cuenta(app):
    ...
    assert "totalizar_hoja_guardada" in src and "hoja_guardada=" in src


def test_la_hoja_guardada_es_solo_lectura():
    """Sin aviso previo, sin corte por fecha, sin Confirmar — pero con Imprimir."""
    assert PREVIEW.count("{% if not hoja_guardada %}") == 2
    assert "window.print()" in PREVIEW
    assert "Hoja del totalizar del {{ hoja_guardada.fecha | fecha_es }}" in PREVIEW
