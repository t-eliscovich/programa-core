...
class _DBRecorder:
    ...
    def __init__(self, *, cheque: dict, aplic: list[dict] | None = None,
...
def test_reversar_B_to_1_flips_cliente_stop(monkeypatch):
    """Cheque depositado en Pichincha (B) que rebota → stat=1 + cliente STOP."""
    import db
    from modules.cheques import queries

    rec = _DBRecorder(
        cheque={
            "id_cheque": 42, "no_cheque": "12345678",
            "stat": "B", "codigo_cli": "JTX",
        },
        aplic=[],
    )
    ...
    assert out["stop_aplicado"] is True
    assert out["stat_previo"] == "B"
    assert out["stat_nuevo"] == "1"
    assert out["codigo_cli"] == "JTX"
    assert _cheque_update_stat(rec.executes) == "1"
    assert _contains_cliente_stop_update(rec.executes), \
        "El UPDATE scintela.cliente SET stop='S' no se emitió."
...
def test_reversar_1_to_3_segundo_rebote(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 43, "no_cheque": "99999",
            "stat": "1", "codigo_cli": "ACM",
        },
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_A_legacy_to_1_flips_stop(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 44, "no_cheque": "LEGACY",
            "stat": "A", "codigo_cli": "OLD",
        },
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_Z_to_X_no_stop(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 45, "no_cheque": "7777",
            "stat": "Z", "codigo_cli": "FOO",
        },
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_P_to_X_no_stop(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 46, "no_cheque": "8888",
            "stat": "P", "codigo_cli": "BAR",
        },
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_D_to_X_no_stop(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 47, "no_cheque": "DANIELA",
            "stat": "D", "codigo_cli": "FOO",
        },
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_idempotent_when_cliente_already_stopped(monkeypatch):
    """Cliente ya en STOP: el UPDATE se emite pero rowcount=0 ⇒ stop_aplicado=False."""
    import db
    from modules.cheques import queries

    rec = _DBRecorder(
        cheque={
            "id_cheque": 48, "no_cheque": "1234",
            "stat": "B", "codigo_cli": "JTX",
        },
        rowcount_cliente_update=0,  # simula WHERE stop!='S' matching nothing
    )
    rec.apply_to(monkeypatch, db)
    monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
    ...
    assert out["es_rebote_real"] is True     # sí fue un rebote real
    assert out["stop_aplicado"] is False      # pero el cliente ya estaba en stop
    # Sí se emitió el UPDATE (la idempotencia vive en el WHERE, no en la lógica Python).
    assert _contains_cliente_stop_update(rec.executes)
...
def test_reversar_terminal_levanta(monkeypatch):
    ...
    for terminal in ("X", "R", "3"):
        rec = _DBRecorder(
            cheque={
                "id_cheque": 49, "no_cheque": "x",
                "stat": terminal, "codigo_cli": "JTX",
            },
        )
        rec.apply_to(monkeypatch, db)
        monkeypatch.setattr(queries, "asegurar_fecha_abierta", lambda _f: None)
...
def test_reversar_observacion_capada_a_200_chars_en_sql(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 51, "no_cheque": "CAPTEST",
            "stat": "B", "codigo_cli": "JTX",
        },
    )
    rec.apply_to(monkeypatch, db)
    ...
    assert queries._OBS_CAP in cliente_update[1], \
        "El cap (200) se pasa como parámetro al RIGHT()."
...
def test_reversar_fecha_de_hoy(monkeypatch):
    ...
    rec = _DBRecorder(
        cheque={
            "id_cheque": 52, "no_cheque": "FECHA",
            "stat": "Z", "codigo_cli": "JTX",
        },
    )
    rec.apply_to(monkeypatch, db)
