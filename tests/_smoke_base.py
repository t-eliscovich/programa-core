...
def app_smoke(dbname: str):
    ...
    _db_name_previo = os.environ.get("DB_NAME")
    os.environ["DB_NAME"] = dbname
    import db as real_db
    ...
    try:
        ...
        os.environ["DB_NAME"] = _db_name_previo or dbname
        # TMT 2026-08-14 — acá antes iba `close_pool()`, y dejaba el pool
        # CERRADO para el test siguiente. Costó un CI rojo: los cinco tests de
        # `test_papelera_pendientes` fallaban con "relation
        # scintela.banco_pendientes_papelera does not exist", porque su tabla
        # nace en `papelera_pendientes._bootstrap()` — que es fail-soft: si el
        # `db.execute` revienta, LOGUEA y marca `_bootstrapped = True` igual.
        # O sea: mi pool cerrado hacía que la tabla no se creara nunca y el
        # error aparecía tres archivos más adelante, en código ajeno.
        # Dejar el pool ABIERTO y apuntando a donde estaba es la única forma de
        # que este barrido no le cambie el piso a nadie.
        try:
            real_db.init_pool()
