...
def _factura_de(data: dict, numf: int, doc: str = "") -> dict:
    ...
    # ⭐ Primero por el NÚMERO COMPLETO, que no se repite nunca (26/08/2026).
    # `numf` es el número corto: se repite —dentro de un mismo cliente hay 13
    # pares, y 288 documentos lo tienen en CERO, casi todos notas de entrega—.
    # Buscando sólo por él, el vendedor abría el PRIMERO de la lista con ese
    # número y podía ser otro documento. Dueña 26/08/2026: *"que hayamos puesto
    # 5 números en vez de todo el número no tiene que perder de vista si es
    # nota de entrega, devolución, factura"*.
    doc = (doc or "").strip()
    if doc:
        for f in facturas:
...
@mi_cartera_bp.route("/mi-cartera/pedidos/cancelar-memo", methods=["POST"])
@requiere_login
@requiere_permiso("micartera.ver")
def pedidos_cancelar_memo():
    """El vendedor cancela el memo PENDIENTE de un pedido SUYO, para poder
    mandarlo de nuevo con datos al día. Mismo cerco de dueño que
    `pedidos_enviar_memo`."""
    vend = _vend_actual()
    numero = (request.form.get("numero") or "").strip()
    detalle = pedidos_service.armar_memo(numero) if numero else None
    if detalle is None:
        flash("No encontré ese pedido entre los pendientes, así que no pude "
              "cancelar el memo. Recargá la pantalla y probá de nuevo.", "error")
        return redirect(_url_mis_pedidos())
    if detalle["vendedor"]["codigo"] != vend:
        abort(404)
