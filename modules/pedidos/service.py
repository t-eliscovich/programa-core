...
# ── Pedidos MUY viejos (+90 días), que quedan fuera del total ─────────────────
# Los pedidos de más de DIAS_PEDIDO_MAX no entran a la cuenta (decisión dueña
# 2026-08-17). Para "darse cuenta" de que existen, esta consulta los CUENTA
# aparte (no los suma) y la pantalla muestra una línea. Fail-soft: {} si falla.
_SQL_MUY_VIEJOS = """
SELECT COUNT(DISTINCT v.numero)                                AS n_pedidos,
       COUNT(DISTINCT v.cliente)                               AS n_clientes,
       SUM(CASE WHEN v.id_unidad_pedido = {rollo}
                THEN v.saldo_comprometido * {kg_rollo}
                WHEN v.id_unidad_pedido = {kg} THEN v.saldo_comprometido
  ...
  JOIN producto pr ON pr.id_producto = v.id_producto
 WHERE DATEDIFF(day, v.fecha, {ahora}) > {dias_ped}
   AND pr.nombre_categoria_producto IN ({cats})
"""


def pedidos_muy_viejos() -> dict:
    """Cuántos pedidos de +DIAS_PEDIDO_MAX días hay (los que NO entran al total).

    Devuelve `{}` si no hay ninguno o si Asinfo no contesta (fail-soft).
    """
    cats = ",".join("'" + c.replace("'", "''") + "'" for c in CATEGORIAS)
    sql = _SQL_MUY_VIEJOS.format(
        rollo=UNIDAD_ROLLO, kg=UNIDAD_KG, kg_rollo=KG_POR_ROLLO,
        ahora=AHORA_EC, dias_ped=DIAS_PEDIDO_MAX, cats=cats)
    filas, ok = metabase_client.fetch_dataset_estado(ASINFO_DB, sql)
    if not ok or not filas:
        return {}
    r = filas[0]
    # La fila de conteo trae la columna `kg`; una fila de `pendientes` (que
    # algunos tests devuelven con un mock global) no — así no se malinterpreta.
    if r.get("kg") is None:
        return {}
    n = int(_f(r.get("n_pedidos")))
    if n == 0:
        return {}
    viejo = str(r.get("mas_viejo") or "")[:10]
    return {
        "n_pedidos": n,
        "n_clientes": int(_f(r.get("n_clientes"))),
        "kg": round(_f(r.get("kg"))),
        "mas_viejo_es": _fecha_corta(viejo),
        "dias": _dias_desde(viejo),
    }
