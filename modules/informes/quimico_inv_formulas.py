...
# TMT 2026-07-24 (dueña "quiero ver lo mismo que formulas"): la columna QUÍM.$
# del flujo tiene que ser IDÉNTICA a la pantalla "TOTALES POR TIPO" de
# formulas_app. Inicial/Final salen del físico exacto (quimico_total_fisico);
# Compras/Ajuste/Consumido se valúan TODOS a CATÁLOGO (productos.us) × IVA ×
# A44/10 — la MISMA vara que el físico — para que la identidad cierre:
#   Inicial + Compras + Ajuste − Consumido = Final
# (antes Compras salía a precio de compra y Consumido a precio de línea → otra
# vara, por eso no cerraba y quedaba ~1.5k corrido).
_A44 = "(CASE WHEN (p.familia ILIKE 'aux' OR p.num < 100) AND p.num_visible = 44 THEN 0.1 ELSE 1.0 END)"
_IVA = "(CASE WHEN p.num = 12 THEN 1.0 ELSE 1.15 END)"
_FAM = "(UPPER(TRIM(p.familia)) IN ('POLI', 'ALG', 'AUX') OR p.num < 300)"

_SQL_COMPRAS_CAT = f"""
SELECT COALESCE(SUM(cm.cantidad * COALESCE(p.us, 0) * {_IVA} * {_A44}), 0) AS us
  FROM compras cm
  JOIN productos p ON p.num = cm.producto_num
 WHERE cm.fecha >= %(desde)s AND cm.fecha <= %(hasta)s
   AND {_FAM}
"""

_SQL_AJUSTES_CAT = f"""
SELECT COALESCE(SUM(ia.cantidad * COALESCE(p.us, 0) * {_IVA} * {_A44}), 0) AS us
  FROM inventario_ajustes ia
  JOIN productos p ON p.num = ia.producto_num
 WHERE ia.fecha >= %(desde)s AND ia.fecha <= %(hasta)s
   AND {_FAM}
"""
...
def quimico_totales_por_tipo(desde: date, hasta: date) -> dict | None:
    """Los 5 números de "TOTALES POR TIPO" (US$ c/IVA) de formulas para el
    período [desde, hasta]:
      inicial   = físico al día ANTERIOR a `desde` (quimico_total_fisico)
      compras   = Σ compras del período a catálogo × IVA × A44/10
      ajuste    = Σ ajustes de inventario del período (idem valuación)
      consumido = Σ consumo de órdenes TERMINADAS del período (idem valuación)
      final     = físico a `hasta` (quimico_total_fisico)
    Todo a la MISMA vara (catálogo) → cierra Inicial+Compras+Ajuste−Consumido=Final.
    None si formulas_db no está (fail-soft)."""
    from datetime import timedelta

    inicial = quimico_total_fisico(desde - timedelta(days=1))
    final = quimico_total_fisico(hasta)
    if inicial is None or final is None:
        return None
    params = {"desde": desde.isoformat(), "hasta": hasta.isoformat()}
    try:
        _c = formulas_db.fetch_one(_SQL_COMPRAS_CAT, params) or {}
        _a = formulas_db.fetch_one(_SQL_AJUSTES_CAT, params) or {}
        _e = formulas_db.fetch_one(_SQL_CONSUMO_CAT, params) or {}
    except Exception as ex:  # noqa: BLE001 -- fail-soft
        _LOG.warning("quimico_totales_por_tipo %s-%s: %s", desde, hasta, ex)
        return None
    return {
        "inicial": float(inicial),
        "compras": float(_c.get("us") or 0),
        "ajuste": float(_a.get("us") or 0),
        "consumido": float(_e.get("us") or 0),
        "final": float(final),
    }
