...
@compras_bp.route("/compras/historico")
@requiere_login
@requiere_permiso("compras.ver")
def historico():
    """Histórico de compras por proveedor — réplica del comando 4B del dBase.

    Igual que en el programa viejo (MODIFICA.PRG / PROCEDURE COMPRAS): se elige
    un PROVEEDOR y una fecha DESDE, y se listan sus compras en columnas
    Fecha · Prov · Kg · Concepto · Importe · Banco, con el total abajo. Los
    códigos especiales del dBase que más se usan: XX = todos, QQ = químicos.
    """
    prov = (request.args.get("prov") or "").strip().upper()
    desde = request.args.get("desde") or None
    hasta = request.args.get("hasta") or None

    # Traducción de los códigos del dBase a los filtros de buscar():
    #   ''  → todavía no se eligió proveedor (mostramos solo el formulario)
    #   XX  → todos los proveedores
    #   QQ  → solo químicos (tipo Q), todos los proveedores
    #   otro→ ese código de proveedor
    filas = None
    total_importe = total_kg = 0.0
    error = None
    if prov:
        q = "" if prov in ("XX", "QQ") else prov
        tipo = "Q" if prov == "QQ" else None
        try:
            filas = queries.buscar(
                q, desde, hasta, limite=2000, vista="todas", tipo=tipo,
            )
            try:
                ...
                pass
            agg = queries.total_buscar(q, desde, hasta, vista="todas", tipo=tipo)
            total_importe = agg["total"]
            total_kg = agg["total_kg"]
        except Exception as e:  # noqa: BLE001
            filas, error = [], str(e)

    if prov and request.args.get("export") == "csv":
        return csv_response(
            filas or [],
            columnas=[
                ("fecha", "Fecha"),
            ...
            filename="compras_historico.csv",
        )
    ...
    return render_template(
        "compras/historico.html",
        filas=filas, prov=prov, desde=desde, hasta=hasta,
        total_importe=total_importe, total_kg=total_kg,
        proveedores_datalist=proveedores_datalist, error=error,
    )
