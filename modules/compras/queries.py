...
def crear(
    *,
    ...
    if pagada and cuenta_norm and cuenta_norm not in CUENTAS_PAGO:
        raise ValueError(
            f"Cuenta de pago inválida: {cuenta!r}. "
            f"Debe ser una de: {', '.join(CUENTAS_PAGO)}."
        )
    ...
    if pago_parcial_f > importe_f + 0.01:
        raise ValueError(
            f"El pago parcial ({pago_parcial_f:.2f}) excede el importe "
            f"de la compra ({importe_f:.2f})."
        )
    ...
    if es_pago_parcial:
        if not cuenta_norm:
            raise ValueError(
                "Pago parcial requiere especificar `cuenta` "
                "(caja, pichincha o internacional)."
            )
    ...
    with db.tx() as conn:
        prov_row = db.fetch_one(
            "SELECT id_proveedor, tipo AS tipo_prov "
        ...
        compra = db.execute_returning(
            ...
            """,
            (
                fecha, prov_row["id_proveedor"], codigo_prov.upper().strip(),
                (tipo or None), (comprobante or None),
                kg, importe_f, numero,
                fechad, (concepto or None),
                (clave or None) and clave[:3],
                no_banco_pago, usuario,
                cuenta_pagada_letra,
            ),
            conn=conn,
        ) or {}
        ...
        if importe_pago_inmediato > 0 and cuenta_norm:
            ...
            concepto_pago = (
                (concepto or comprobante or
                 f"Compra #{numero} {codigo_prov}") + sufijo_concepto
            )[:50]
            if cuenta_norm == "caja":
                import caja_helpers
            ...
            if id_transaccion is not None:
                db.execute(
                    "UPDATE scintela.compra SET id_transaccion=%s "
                    "WHERE id_compra=%s",
                ...
                tipo_md = ("compra_pago_parcial" if es_pago_parcial
                           else f"compra_pagada_{cuenta_norm}")
                _md.registrar(
                    conn=conn,
                    tipo=tipo_md,
                    origen_table="compra",
                    ...
                    importe=importe_pago_inmediato,
                    fecha=fecha,
                    concepto=(concepto or comprobante or
                              f"Compra #{numero} {codigo_prov}")[:200],
                    usuario=usuario,
                    metadata={"codigo_prov": codigo_prov,
        ...
        if saldo_posdat > 0.01:
            ...
            if es_pago_parcial:
                concepto_posdat = (
                    f"saldo {codigo_prov} #{numero} "
                    f"(pagué {importe_pago_inmediato:.0f})"
                )[:50]
            posdat_row = db.execute_returning(
                """
                ...
                """,
                (
                    fecha, fechad, codigo_prov.upper().strip(),
                    numero, saldo_posdat, concepto_posdat,
                    (clave or None) and clave[:3],
                    usuario,
                ),
                conn=conn,
            ...
            if posdat_row.get("id_posdat"):
                import mov_doble as _md
                tipo_md = ("compra_saldo_a_posdat" if es_pago_parcial
                           else "compra_a_posdat")
                _md.registrar(
                    conn=conn,
                    tipo=tipo_md,
                    origen_table="compra",
...
def editar(
    ...
    *,
    ...
    if importe is not None and float(importe) != float(compra["importe"] or 0):
        if pagada:
            raise ValueError(
                "Compra ya pagada — el importe no se puede editar. "
                "Para corregir, anular y reemitir."
            )
    ...
    if fechad is not None and fechad != compra["fechad"]:
        if pagada:
            raise ValueError(
                "Compra ya pagada — la fecha de vencimiento no se puede editar."
            )
        nuevo_fechad = fechad
        ...
        if not pagada:
            db.execute(
                ...
                (
                    ...
                    (nuevo_importe - parcial_pagado) if tiene_pago_parcial_activo
                    else nuevo_importe,
                    nuevo_fechad,
                    usuario,
                    compra["codigo_prov"],
    ...
    with db.tx() as conn:
            ...
            db.execute(
                ...
                (
                    (f"compra anulada #{compra.get('numero')} — {motivo}"[:200]
                     if motivo
                     else f"compra anulada #{compra.get('numero')}")[:200],
                    usuario[:50],
                    compra["codigo_prov"], compra["numero"],
                ),
                conn=conn,
        ...
        if cuenta in ("C", "P") and compra.get("id_transaccion") and not compra.get("no_banco"):
            ...
            row_caja = db.fetch_one(
                ...
                (compra["id_transaccion"],), conn=conn,
            )
            if row_caja:
        ...
        if md_usd and md_usd.get("destino_id"):
            ...
            if row_usd:
                ...
                side_effect_resumen.append(
                    f"USD -${abs(float(row_usd.get('importe') or 0)):.2f}"
                )
...
def convertir_anticipo(
    ...
    *,
    ...
    if tipo_actual != "A":
        raise ValueError(
            f"Sólo se pueden convertir anticipos. Tipo actual: {tipo_actual!r}."
        )
...
def buscar(
    ...
    rows_asc = sorted(rows, key=lambda r: (r.get("fecha") or date.min,
                                            r.get("id_compra") or 0))
    acum = 0.0
    for r in rows_asc:
...
def conteos_por_vista(
    ...
    out: dict = {r["bucket"]: {"n": int(r["n"] or 0),
...
def deuda_proveedores() -> dict:
    ...
    if not row:
        return {"total": 0.0, "n_partidas": 0, "n_proveedores": 0,
