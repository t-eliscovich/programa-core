...
# ---------------------------------------------------------------------------
# COBRO EN EFECTIVO / EFT — aplica a factura(s) sin pasar por cheque.
# TMT 2026-05-19 v8 — pedido dueña: "agrega esa logica para cuando un
# cliente paga en efectivo una factura. despues sirve para calcular
# comisiones tambien". Códigos legacy del dBase:
#   99 → efectivo, va a scintela.caja
#   90 → EFT a Pichincha
#   91 → EFT a Internacional
# ---------------------------------------------------------------------------

def crear_cobro_efectivo(
    *,
    codigo_cli: str,
    fecha: date,
    importe_total: float,
    destino: str,  # "99"=caja, "90"=Pichincha, "91"=Internacional
    aplicaciones: list[dict],  # [{id_factura, monto}, ...]
    concepto_libre: str = "",
    usuario: str = "web",
) -> dict:
    """Registra atómicamente: entrada en caja/banco + abonos a facturas +
    fila en scintela.cobro (para que sume en comisiones).

    Args:
        codigo_cli: 3 letras del cliente.
        fecha: fecha del cobro.
        importe_total: monto total recibido (USD).
        destino: '99' (caja), '90' (Pichincha), '91' (Internacional).
        aplicaciones: lista de {id_factura, monto} — la suma debe igualar
                      importe_total.
        concepto_libre: texto extra para el concepto.
        usuario: username del operador.

    Returns:
        {ok, n_facturas, importe, id_caja_o_no_banco, codigos_cancelados}
    """
    from datetime import date as _date
    ...
    if not codigo_cli or len(codigo_cli) < 2:
        raise ValueError("Código de cliente requerido.")
    codigo_cli = codigo_cli.strip().upper()
    fecha = fecha or _date.today()
    importe_total = float(importe_total or 0)
    if importe_total <= 0:
        raise ValueError("Importe total debe ser > 0.")
    if destino not in ("99", "90", "91"):
        raise ValueError(f"Destino inválido: {destino!r}. Usar 99/90/91.")
    if not aplicaciones:
        raise ValueError("Debe aplicarse a al menos una factura.")
    suma_apps = sum(float(a.get("monto") or 0) for a in aplicaciones)
    if abs(suma_apps - importe_total) > 0.02:
        raise ValueError(
            f"La suma de aplicaciones ({suma_apps:.2f}) no coincide "
            f"con el importe total ({importe_total:.2f})."
        )

    destino_label = {
        "99": "EFECTIVO (CAJA)",
        "90": "EFT PICHINCHA",
        "91": "EFT INTERNACIONAL",
    }[destino]
    concepto_caja = f"{destino} {codigo_cli} {concepto_libre or destino_label}".strip()[:80]

    cancelados: list[int] = []

    with _db.tx() as conn:
        # 1) Entrada en caja (siempre se registra, sea efectivo o EFT).
        # Para efectivo (99) suma al saldo de caja física.
        # Para EFT (90/91) la usamos como registro contable trazable; el
        # impacto al banco lo dejamos para una segunda iteración cuando
        # tengamos clara la integración con scintela.transacciones_bancarias.
        _db.execute(
            """
            INSERT INTO scintela.caja
                (fecha, tipo, importe, concepto, usuario_crea)
            VALUES (%s, 'E', %s, %s, %s)
            """,
            (fecha, importe_total, concepto_caja, usuario),
            conn=conn,
        )

        # 2) Aplicar abono a cada factura usando facturas.editar (existente).
        for app in aplicaciones:
            id_fact = int(app["id_factura"])
            monto = float(app["monto"])
            if monto <= 0:
                continue
            # Levantar abono actual + importe
            fact = _db.fetch_one(
                "SELECT importe, abono FROM scintela.factura WHERE id_factura = %s",
                (id_fact,),
                conn=conn,
            ...
            if not fact:
                raise ValueError(f"Factura {id_fact} no existe.")
            abono_actual = float(fact["abono"] or 0)
            importe_fact = float(fact["importe"] or 0)
            abono_nuevo = round(abono_actual + monto, 2)
            if abono_nuevo > importe_fact + 0.01:
                raise ValueError(
                    f"Factura {id_fact}: el abono nuevo ({abono_nuevo:.2f}) "
                    f"excede el importe ({importe_fact:.2f})."
                )
            # facturas.editar maneja el cálculo de stat (T/A/Z) y vencimiento.
            _fact_q.editar(
                id_fact, abono=abono_nuevo,
                observacion=f"Pago {destino_label} {codigo_cli}",
                usuario=usuario,
            )
            if abono_nuevo >= importe_fact - 0.01:
                cancelados.append(id_fact)

            # 3) Registrar en scintela.cobro (para comisiones).
            _db.execute(
                """
                INSERT INTO scintela.cobro
                    (codigo_cli, valor, fecha, banco, tipo_doc,
                     no_docpago, id_factura, usuario_crea)
                VALUES (%s, %s, %s, %s, 'EFT', %s, %s, %s)
                """,
                (
                    codigo_cli, monto, fecha,
                    destino_label, int(destino), id_fact, usuario,
                ),
                conn=conn,
            )
    ...
    return {
        "ok": True,
        "n_facturas": len(aplicaciones),
        "importe": importe_total,
        "codigos_cancelados": cancelados,
        "destino": destino,
    }
